...
yaml_file=$DATA_DIR/data/iwslt2021/txt/iwslt2021.yaml
shift=2
...
cat $outfile.temp | $WORKING_DIR/onlinization.py \
  ...
  STOP $DATA_DIR/data/iwslt2021/txt --task speech_text_joint_to_text --path $model \ # start of model parameter config
  ...
  --scoring sacrebleu --lenpen 1.0 --quiet --infer-target-lang $TRGLANG > $outfile.stdout 2> $outfile.stderr
